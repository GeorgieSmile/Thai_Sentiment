...
ytForm.addEventListener('submit', async (event) => {
    ...
    try {
        const response = await fetch("http://localhost:8000/sentiment/youtube", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ url: ytInput.value.trim() })
        });
        ...
        displayResult("YouTube Video", now);
...
function displayResult(title, uploadTime) {
  metadataDiv.innerText = `วิดีโอ: ${title} | ความคิดเห็นทั้งหมด: ${originalData.length} | วิเคราะห์เมื่อ: ${uploadTime}`;
